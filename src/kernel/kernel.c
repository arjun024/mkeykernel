...
void event_handler (void)
{
	...
	for (;;)
	{
		if (message_read (&msg, &sender) == 0)
        {
			if (msg == MSG_EVENT_KEY)
			{
				// kprint ("event_handler: got msg"); kprint_newline ();

				message_send (keyboard_ch, sender, 1);
			}
		}
	}
}
...
void run_event_handler (void)
{
	// must be started as thread 0!!!

	uint8_t *base = (uint8_t *) kmalloc (4096 * 2);
    uint8_t *event_context = (uint8_t *) kmalloc (4096);
    uint8_t *event_stack = (uint8_t *) kmalloc (4096 * 2);

    thread_init(base);
	thread_create(event_context, event_stack, 4096 * 2, (uint32_t)event_handler, 0, "event_handler");
}
...
{
    uint32_t ticks;thread_sender_pid = current_msg->thread_sender_num;
    uint32_t ticks_max = clock () + 10000;

    // normal priority = 0
	for(;;)
	{
        ticks = clock ();thread_sender_pid = current_msg->thread_sender_num;
...
void run_threads (void)
{
    ...
    uint8_t *thread_b_cothread_sender_pid = current_msg->thread_sender_num;ntext = (uint8_t *) kmalloc (4096);
...
void thread_b_backgr(uint32_t argument)
{
	...
	for(;;)thread_sender_pid = current_msg->thread_sender_num;
	{
        ticks = clock ();
		kdelay (5);
...
void kmain (multiboot_info_t* mbt, unsigned int magic)
{
    ...
    */
	...
	if (pmem_set_bitmap (0x000000, 0x2FFFFFF, 0, ALLOCATE) != MEM_ERR_OK)
	{
		kprint ("MEMORY ERROR: mark module load reserved");
		kprint_newline ();
	}
